...
async def async_setup_entry(hass: HomeAssistant, entry, async_add_devices):
    ...
    async_add_devices(
        [
            CompitClimate(coordinator, device, device_definition.parameters, device_definition.name)
...
class CompitClimate(CoordinatorEntity, ClimateEntity):
    ...
    def __init__(self, coordinator : CompitDataUpdateCoordinator, device: Device, parameters: List[Parameter], device_name: str):
        super().__init__(coordinator)
        self.coordinator = coordinator
        self.unique_id = f"{device.label}_climate"
        self.label = f"{device.label} climate"
        self.parameters = {parameter.parameter_code: parameter for parameter in parameters}
        self.device = device
        ...
        self.available_hvac_modes: Parameter = self.parameters.get('__trybpracyinstalacji')
        self.device_name = device_name
        self.set_initial_values()

    def set_initial_values(self):
        preset_mode = self.coordinator.data[self.device.id].state.get_parameter_value("__trybpracytermostatu")
        if preset_mode is not None:
            self._preset_mode = next((item for item in self.available_presets.details if item.state == preset_mode.value), None).state
        else:
            self._preset_mode = None
        fan_mode = self.coordinator.data[self.device.id].state.get_parameter_value("__trybaero")
        if fan_mode is not None:
            self._fan_mode = next((item for item in self.available_fan_modes.details if item.state == fan_mode.value), None).state
        else:
            self._fan_mode = None

        hvac_mode = self.coordinator.data[self.device.id].state.get_parameter_value("__trybpracyinstalacji")
    ...
    def current_temperature(self):
        value = self.coordinator.data[self.device.id].state.get_parameter_value('__tpokojowa')
        if value is None:
            return None
        return value.value
    ...
    def target_temperature(self):
        value = self.coordinator.data[self.device.id].state.get_parameter_value('__tpokzadana')
        if value is None:
            return None
        return value.value
    ...
    def preset_mode(self):
        ...
        return next((item for item in self.available_presets.details if item.state == self._preset_mode), None).description
    ...
    def fan_mode(self):
        ...
        return next((item for item in self.available_fan_modes.details if item.state == self._fan_mode), None).description
    ...
    async def async_set_preset_mode(self, preset_mode):
        ...
        value = next((item for item in self.available_presets.details if item.description == preset_mode), None)
        self._preset_mode = value.state
        await self.async_call_api("__trybpracytermostatu", value.state)

    async def async_set_fan_mode(self, fan_mode):
        """Set new target fan mode."""
        value = next((item for item in self.available_fan_modes.details if item.description == fan_mode), None)
    ...
    async def async_call_api(self, parameter: str, value: int) -> None:
        try:
            if await self.coordinator.api.update_device_parameter(self.device.id, parameter, value) != False:
